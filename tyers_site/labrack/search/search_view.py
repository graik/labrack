...
from labrack.search.search_form import MultipleItemSearchForm
...
def basic_search(request, template='search/search.html', load_all=True, form_class=MultipleItemSearchForm, searchqueryset=None, context_class=RequestContext, extra_context=None, results_per_page=None):
    """
...
"""
    ...
    if request.GET.get('q'):
        ...
        if form.is_valid():
            ...
            marker_Search = form.cleaned_data['marker_Search']
            if (marker_Search=='include'):
                results = SearchQuerySet().filter(content=query)
            else :
                results = SearchQuerySet().exclude(content=marker).filter(content=query)
            #results = form.search()
    else:
